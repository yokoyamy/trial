...
function githubChecked(key) {
    return Boolean(
        githubElement(key)?.checked
    );
}


function setGithubValue(key, value = '') {
    const element =
        githubElement(key);
...
function setGithubChecked(key, value) {
    const element =
        githubElement(key);
...
function isMockOnly(node) {
    ...
    const hasDraft =
        children.some(child =>
            child.name === 'draft' ||
            /\/draft$/i.test(child.path || '')
        );

    const hasMock =
        children.some(child =>
            child.name === 'mock' ||
            /\/mock$/i.test(child.path || '')
        );
...
function buildTreeNode(
    ...
    } else if (childNodes(node).length) {
        childrenHTML =
            buildHTML(
                node.children,
                {
    ...
    return `
        ...
        >
            <summary
                draggable="true"
                ondragstart="typeof drag === 'function' && drag(event, '${safePath}')"
                ondragover="typeof allowDrop === 'function' && allowDrop(event)"
                ondragleave="typeof dragLeave === 'function' && dragLeave(event)"
                ondrop="typeof drop === 'function' && drop(event, '${safePath}')"
                oncontextmenu="typeof showContext === 'function' && showContext(event, '${safePath}', ${published})"
                onclick="typeof selectFolder === 'function' && selectFolder(event, '${safePath}', ${published})"
            >
                <span
    ...
    return `
        ...
        >
            <summary
                ...
                ondragleave="typeof dragLeave === 'function' && dragLeave(event)"
                ondrop="typeof drop === 'function' && drop(event, '${safePath}')"
                oncontextmenu="typeof showContext === 'function' && showContext(event, '${safePath}', ${isPublic})"
                onclick="typeof selectFolder === 'function' && selectFolder(event, '${safePath}', ${isPublic})"
            >
                <span
                    style="margin-right:6px;display:inline-block;width:18px;text-align:center;"
                >${icon}</span>
...
async function selectFolder(
    ...
    if (
        typeof window.setAppGitHubSyncTarget ===
        'function'
    ) {
    ...
    const isExecutable =
        ...
        activeEl?.dataset.type === 'app-item';
...
function createHistoryItem(item, path) {
    const filename =
        item.filename ||
        item.name ||
        '';
...
/*
 * HTML側では btn-save-current になっている。
 * 旧コードとの互換性も残す。
 */
const btnSave =
    $('btn-save-current') ||
...
function stopBlocking() {
    ...
    /*
     * visible クラスで表示制御している場合は
     * inline display を勝手に消さない。
     */
}
...
function buildHTML(nodes, options = {}) {
    ...
    /*
     * 公開ツリーでは published 自体を
     * ノードとして出さない。
     *
     * 開発ツリーでは draft/mock の実体を
     * アプリ配下に再構成する。
     */
    return nodes
        .filter(node => {
...
function buildTreeNode(
    ...
    const safePath =
        escapeAttribute(appBasePath);

    const jsPath =
        JSON.stringify(
            normalizePath(appBasePath)
        );
    ...
    return `
        <details
            ...
            class="${isApp ? 'item-app-group' : 'item-folder'}"
            open
            ontoggle="if(typeof saveTreeState==='function'){saveTreeState();}"
        >
            <summary
                draggable="true"
                ondragstart="if(typeof drag==='function'){drag(event, ${jsPath});}"
                ondragover="if(typeof allowDrop==='function'){allowDrop(event);}"
                ondragleave="if(typeof dragLeave==='function'){dragLeave(event);}"
                ondrop="if(typeof drop==='function'){drop(event, ${jsPath});}"
                oncontextmenu="if(typeof showContext==='function'){showContext(event, ${jsPath}, ${published});}"
                onclick="if(typeof selectFolder==='function'){selectFolder(event, ${jsPath}, ${published});}"
            >
                <span
                    style="margin-right:6px;display:inline-block;width:18px;text-align:center;"
                >${icon}</span>
...
function buildAppStages(
    ...
    /*
     * 必ず「開発中」と「モック」を生成する。
     * これがツリーに stage が出ない問題の
     * 主原因になっていた構造を固定する。
     */
    html += createStageItem(
        draftPath,
        '🔨',
        '開発中',
        published,
    ...
    return `
        ...
        >
            <summary
                ...
                ondragleave="if(typeof dragLeave==='function'){dragLeave(event);}"
                ondrop="if(typeof drop==='function'){drop(event, ${jsPath});}"
                oncontextmenu="if(typeof showContext==='function'){showContext(event, ${jsPath}, ${isPublic});}"
                onclick="if(typeof selectFolder==='function'){selectFolder(event, ${jsPath}, ${isPublic});}"
            >
                <span
                    style="margin-right:6px;display:inline-block;width:18px;text-align:center;"
                >${icon}</span>
...
async function selectFolder(
    ...
    /*
     * summary のデフォルト動作を止める。
     * ただし details 自体は開閉可能にする。
     */
    if (
        eventOrTarget instanceof Event &&
        eventOrTarget.target instanceof Element
    ) {
        const summary =
            eventOrTarget.target.closest(
                'summary'
            );

        /*
         * summaryクリック時は
         * select と details の toggle を両立。
         */
    }
...
async function initializeGojacic() {
    updateLineNumbers();
    ...
    if (
        selectAllButton &&
        !selectAllButton.dataset.jsBound
    ) {
 ...
 * グローバル公開
 *
 * HTML の inline handler から呼ばれる関数は
 * window に明示的に公開する。
 * ========================================================== */
...
window.deleteCurrentPath =
    deleteCurrentPath;

window.renameCurrentPath =
    renameCurrentPath;
...
window.downloadModalContent =
    downloadModalContent;

window.loadPromptHistory =
    loadPromptHistory;

window.selectPromptHistory =
    selectPromptHistory;
